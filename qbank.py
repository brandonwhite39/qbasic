...
def main():
    ...
    transactionFile = args.transactions
    ...
    with open(transactionFile, 'w') as transactions:
        for i in transList:
...
def createacct(accountList,transList): 
    ...
    while notCreated == True:
        ...
        if (len(newAccNum) == 7) and (int((newAccNum)[:1]) != 0) and (int(newAccNum) not in accountList):
            ...
            if (newAccNam.isalpha() == True) and len(newAccNam) > 2 and len(newAccNam) < 31:
                ...
                notCreated = False
                print(accountList)
                return accountList, transList.append("NEW " + str(newAccNum) + " 000 0000000 " + newAccNam)
            else:
...
def deleteacct(accountList,transList):
    ...
    while notDeleted == True:
        ...
        #check all the constraints
        if (len(delAccNum) == 7) and (int((delAccNum)[:1]) != 0) and (int(delAccNum) in accountList):
            ...
            print(accountList)
            return accountList, transList.append("DEL 0000000 000 " + str(delAccNum) + " ***")
...
#translist as output
def deposit(accntList,mode, transList):
    validAcc = False
    validDep = False
        ...
        if notNumber == True:
            ...
            if mode == 'm' or mode == 'M':
                if withAmt < 0:
                    ...
                    validWith = True
                    totalWith += withAmt
                    complete = input("Enter 1 to complete withdraw, Enter 2 to add another withdraw")
                    if complete == '1':
                        if totalWith < 100000 and totalWith > 0:#positive testing
                ...
                if withAmt > 99999999:
                    ...
                    complete = input("Enter 1 to complete deposit, Enter 2 to add another deposit")
                    if complete == '1':
                        if totalWith < 99999999:#positive testing
                            return transList.append("WDR "+ str(accNum)+ " " + str(totalWith) + " " + "0000000 ***")
                            completeWith = True
                        else:
                            print("Invalid total withdraw amount")
        ...
        if (int(fromAcc) in accountList) and (int(toAcc) in accountList):
            ...
            if(currentMode == 'M'):
                ...
                if(transAmount >= 0) and (transAmount <= 99999999):
                    newTransaction = "XFR " + str(toAcc) + " " + str(transAmount) + " " + str(fromAcc) + " " + "***"
                    confirmTrans == False
                    transList.append(newTransaction)
                    return transList
                else:
